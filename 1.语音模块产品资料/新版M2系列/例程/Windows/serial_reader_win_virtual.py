...
class SerialMessageReader:
    ...
    def open_serial(self) -> bool:
        """打开串口"""
        try:
            # Windows下需要设置不同的timeout参数
            timeout = 1.0 if os.name == 'posix' else 0.5
            ...
            print(f"串口打开失败: {e}")
            if "Permission denied" in str(e) and os.name == 'posix':
                print("提示: 可能需要使用sudo或添加用户到dialout组")
            elif "FileNotFoundError" in str(e):
                print("提示: 串口设备不存在，请检查设备连接和端口号")
            return False
    ...
    def handle_audio_data(self, message_data: bytes):
        ...
        try:
            with open(self.pcm_file, 'ab') as f:
                ...
                # 可选：实时显示已保存的音频数据大小
                if self.message_count % 10 == 0:  # 每10条消息显示一次
    ...
    def print_message_info(self, header_info: dict, message_data: bytes):
        ...
        if message_data:
            # 如果是音频数据，只显示音频信息，不尝试打印文本
            if header_info['msg_type'] == self.AUDIO_DATA_TYPE:
                print(f"音频数据: {len(message_data)} 字节")
                # 可选：显示更多音频信息，如采样率、时长等
                # print(f"音频格式: {self.get_audio_format(message_data)}")
                return  # 直接返回，不再尝试打印文本
            
            # 非音频数据，尝试打印文本内容
            try:
                text_data = message_data.decode('utf-8', errors='ignore')
                # 检查是否有可打印字符（排除全是控制字符的情况）
                ...
                print(f"数据(二进制): {len(message_data)} 字节 (解码失败: {e})")
...
def main():
    parser = argparse.ArgumentParser(
        description='串口消息读取与验证工具 - Windows/Linux兼容版',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  %(prog)s -p COM3 -b 115200
  %(prog)s -p /dev/ttyUSB0 -b 9600
  %(prog)s --list-ports
        """
    )
    parser.add_argument('-p', '--port', 
                       default='COM16' if os.name == 'nt' else '/dev/ttyACM0',
                       help='串口设备路径 (Windows默认: COM1, Linux默认: /dev/ttyACM0)')
