...
:rclcpp::Node(node_name),serial_initialized(false){
	...
	/* 手动唤醒触发订阅（前端 → ROS → M2 串口） */
	wakeup_trigger_sub_ = this->create_subscription<std_msgs::msg::String>(
		"wheeltec_mic/wakeup_trigger", 10,
		[this](const std_msgs::msg::String::SharedPtr msg) {
			RCLCPP_INFO(this->get_logger(), ">>>>>收到前端唤醒触发: %s", msg->data.c_str());
			if (!serial_initialized) {
				RCLCPP_ERROR(this->get_logger(), ">>>>>串口未初始化，无法发送唤醒指令");
				return;
			}
			std::string json = "{\"type\":\"manual_wakeup\",\"content\":{\"beam\":0}}";
			unsigned short sid = static_cast<unsigned short>(
				this->now().nanoseconds() & 0xFFFF);
			std::string packet = MakeMsgPacket(sid, MsgType::CONTROL, json);
			try {
				MicArr_Serial.write(packet);
				RCLCPP_INFO(this->get_logger(), ">>>>>已发送手动唤醒指令给M2");
			} catch (const serial::IOException& e) {
				RCLCPP_ERROR(this->get_logger(), ">>>>>发送手动唤醒指令失败: %s", e.what());
			}
		});
	initialize_serial();
