...
std::string awake_feedback = "请问有什么指示。 ";
            ...
            if (sub == "tts") {
                ...
                if (reader.parse(resultStr, resultJson, false)) {
                    ...
                    if (isLast) {
                        ...
                        node->voice_words_pub.publish(msg);
                        mIatTextBuffer.clear();
                    }
                }
            } 
                ...
                if (reader.parse(resultStr, resultJson, false)) {
                    ...
                    std::cout << "esr_iat: " << mIatTextBuffer << std::endl;
 ...
 *
 */
void initializePaths(const std::string& source_path) {
    TEST_ROOT_DIR_PATH = source_path + TEST_ROOT_DIR;
    CFG_FILE_PATH = source_path + CFG_FILE;
    TEST_AUDIO_PATH = source_path + TEST_AUDIO;
    LOG_DIR_PATH = source_path + LOG_DIR;
    MSC_DIR_PATH = source_path + MSC_DIR;
    TEST_TTS_PATH = source_path + TEST_TTS;
    TEST_SEE_SAY_PATH = source_path + TEST_SEE_SAY;
}
 ...
 * @param cfgPath
 */
void createAgent(bool more = true, const char* cfgPath = CFG_FILE_PATH.c_str())
{
    if (g_pAgent) {
        return;
    }
...
void writeAudioFromLocal(bool repeat)
{
    ...
    ifstream testData(TEST_AUDIO_PATH.c_str(), std::ios::in | std::ios::binary);
...
static void initSetting(bool log = true)
{
    ...
    AIUISetting::setAIUIDir(TEST_ROOT_DIR_PATH.c_str());
    AIUISetting::setMscDir(MSC_DIR_PATH.c_str());
    AIUISetting::setNetLogLevel(log ? aiui_debug : aiui_none);

    long file_size = FileSize(LOG_DIR_PATH.c_str());
    if (file_size == -1) {
...
返回  值：无
**************************************************************************/
void AIUI_Node::tts_Callback(const std_msgs::String& msg){
    tts_text = msg.data;
    startTTS(tts_text);
}
...
返回  值：无
**************************************************************************/
void AIUI_Node::awake_Callback(const std_msgs::Int8 msg){
    awake_flag = msg.data;
...
AIUI_Node::AIUI_Node() {
    ROS_INFO("aiui_node init!");

    ros::NodeHandle nh_("~");
    nh_.param("source_path", source_path, std::string(" "));
    nh_.param("RECORD_DEVICE_NAME", RECORD_DEVICE_NAME, std::string("default"));

    initializePaths(source_path);

    /***识别文本话题发布者创建***/
    voice_words_pub = nh_.advertise<std_msgs::String>("/voice_words", 10);
    /***TTS输出文本话题订阅者创建***/
    tts_words_sub = nh_.subscribe("/feedback_words", 10, &AIUI_Node::tts_Callback, this);
    /***唤醒标志位话题订阅者创建***/
    awake_flag_sub = nh_.subscribe("/awake_flag", 1, &AIUI_Node::awake_Callback, this);
}
...
AIUI_Node::~AIUI_Node(){
    ...
    destroyAgent();
    ROS_INFO("AIUI_Node over!");
}

int main(int argc, char** argv)
{
    ros::init(argc, argv, "aiui_node");
    node = std::make_shared<AIUI_Node>();
